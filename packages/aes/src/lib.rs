...
use galios_tables::{GMUL_11, GMUL_13, GMUL_14, GMUL_9};
pub use galios_tables::{GMUL_2, GMUL_3};
use sbox::INV_SBOX_TABLE;
...
pub struct Aes<const NK: usize, const NR: usize, const K: usize, const E: usize>;

static RCON_RC: [u8; 10] = [1, 2, 4, 8, 16, 32, 64, 128, 27, 54];

fn to_arr_range<'a, T, const N: usize>(
    slice: &'a [T],
...
impl<const NK: usize, const NR: usize, const K: usize, const E: usize> Aes<NK, NR, K, E> {
    // K is the key size in bytes
    // E is the Expanded Key buffers in bytes
    fn key_expansion(key: &[u8; K]) -> [u8; E] {
        assert_eq!(
            K / 4,
            NK,
    ...
    fn cipher(block: &mut [u8; 16], round_keys: &[u8; E]) {
    ...
    fn inv_cipher(block: &mut [u8; 16], round_keys: &[u8; E]) {
